...
print(f'Alan es un chingon,,,,{p.firstname},{p.lastname},{p.age},{p.country},{p.city}')
...
print(p.firstname)      # p.... has access to all functions in Person because p = Person('Asabeneh', 'Yetayeh', 250, 'Finland', 'Helsinki')
...
Sometimes, you may want to have default values for your object methods. 
...
properties. Child class is the class that inherits from another or parent class. Let us create a student class by inheriting 
from person class.
...
We can add a new method to the child or we can override the parent class methods by creating the same method name 
in the child class. When we add the init() function, the child class will no longer inherit the parent's init() function.
...
class Student(Person):
    def __init__(self, firstname='Asabeneh', lastname='Yetayeh',
                 age=250, country='Finland', city='Helsinki', gender='female'):
        self.gender = gender
...
We can use super() built-in function or the parent name Person to automatically inherit the methods and properties 
from its parent. In the example above we override the parent method. The child method has a different feature,
it can identify, if the gender is male or female and assign the proper pronoun(He/She).
